...
class Client {
	...
	getInsights () {
		return this.insights.filter( insight => insight.enabled );
	}

}

// Get client from the database
Client.getByAPIKey = async function getByAPIKey ( key ) {
	...
	let query = [ {
		"apiKey.signature": key
	} ];
	query = query.concat( { projection: { slugName: 1 } } );
