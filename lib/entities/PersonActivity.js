...
class PersonActivity {
	...
	associatePerson () {
		return this
				.setPersonId( arguments[ 0 ] )
				.setPersonPhoneNumber( arguments[ 1 ] )
	}
...
PersonActivity.prototype.getMostRecent = async function getMostRecent () {
	...
	let { cursor, numberOfRecords } = await PersonActivity.get( query );
	if ( ! numberOfRecords )
		return { };
...
// Get the activity that occurred in the last `n` minutes from the database
// PersonActivity.prototype.getMostRecent = async function getMostRecent () {
// 	let query = {
// 		filter: {
// 			event: this.event,
