...
async function getCustomerByInternalId ( client, id ) {
	...
	let customer;
	customer = await makeAPIRequest( `Contacts/${ id }` );

	if ( ! customer )
		customer = await makeAPIRequest( `Leads/${ id }` );
...
async function getCustomerByPhoneNumbers ( client, phoneNumbers ) {
	...
	if ( customer ) {
		...
		customer.externalId = customer.UID || customer.Hidden_UID;
	}
...
async function addCustomer ( person ) {
		...
		data: [
			{
				...
				Mobile: person.contact.otherPhoneNumbers[ 0 ] || "",
				Home_Phone: person.contact.otherPhoneNumbers[ 1 ] || "",
				Asst_Phone: person.contact.otherPhoneNumbers[ 2 ] || "",
				Other_Phone: person.contact.otherPhoneNumbers[ 3 ] || "",
				Secondary_Email: person.contact.otherEmailAddresses[ 0 ] || ""
			}
		],
	...
	let customer;
	customer = await getCustomerByInternalId( internalId );

	if ( ! customer )
		return false;

	let externalId = customer[ "Hidden_UID" ];
