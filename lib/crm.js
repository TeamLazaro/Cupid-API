...
// Renew the API keys right away for good measure
    // The actual function is defined later in this file
 ...
 *
 */
async function getNotification ( channelId ) {
	...
	let response = await makeAPIRequest( `actions/watch`, requestBody );
 ...
 *
 */
async function setNotification ( channelId ) {
		...
		watch: [
			{
				...
				events: [ "Contacts.all" ],
				notify_url: "https://67c814ef.ngrok.io/v1/audit-log"
			}
		]
	};

	let response = await makeAPIRequest( `actions/watch`, requestBody, "POST" );
