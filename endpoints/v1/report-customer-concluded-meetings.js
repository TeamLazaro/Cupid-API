...
function main ( router, middleware ) {
	...
	router.post( "/v1/customers/reports/concluded-meeting", async function ( req, res ) {
		...
		// Respond back
		res.json( { message: "The meeting will be reported." } );
		res.end();
		...
		let message = `Congratulations ${ salesperson.first_name } (<@${ salespersonSlackId }>),\n on just concluding a meeting with _${ customerName }_ [${ customerExternalId }]!`;
		...
		try {
			...
			await log.toUs( {
				context: "Reporting a customer's concluded meeting with a salesperson to the salepeople",
				message: "\n```\n" + e.stack + "\n```"
			} );
		}
