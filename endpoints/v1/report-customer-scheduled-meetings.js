...
function main ( router, middleware ) {
	...
	router.post( "/v1/customers/reports/scheduled-meeting", async function ( req, res ) {
		...
		let message = `${ salesperson.first_name } (<@${ salespersonSlackId }>),\n you have a meeting scheduled with _${ customerName }_ [${ customerExternalId }].`;
		...
		try {
			...
			await log.toUs( {
				context: "Reporting a customer's scheduled meeting with a salesperson to the salepeople",
				message: "\n```\n" + e.stack + "\n```"
			} );
		}
